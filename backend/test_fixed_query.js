...
async function testFixedQuery() {
    try {
        console.log('Testing FIXED query for parcel ID: 28556\n');

        // This is the FIXED query from index.js
        const query = `
      SELECT
        p.id,
        p.num_parcel,
    ...
    `;
        ...
        console.log('  - Num Parcel:', result.rows[0].num_parcel);
        console.log('  - Type:', result.rows[0].type);
        console.log('  - Status:', result.rows[0].status);
        console.log('  - Village:', result.rows[0].village);
        console.log('\nDetails object keys:', Object.keys(result.rows[0].details || {}));
